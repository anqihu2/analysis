...
data_list <-
  list.files(path = "Splash/cloze_analysis/raw_data",
...
# Check trial numbers for each participant. Not applicable if participants completed different numbers of stories.
...
```
**Accuracy lower than 50% before typo is removed: `r prelim_outlier`** Same as the incomplete data. Removed.
...
write.csv(cdata, "Splash/cloze_analysis/typo.csv")
```
...
**Manual checking and cleaning of not identified typos**
**Save manually cleaned typo and accuracy to typo_checked.csv**
...
# Separate Accuracy by Noun/ Verb conditions
```{r}
noun_condition <- read.csv("Splash/cloze_analysis/stim_word/noun_cond_key.csv")

cdataFinal <- merge(cdataFinal, noun_condition, by = c("story", "trial"), all.x = T)
...
# Accuracy by condition modeling
```{r}
acc_noun <- cdataFinal[which(cdataFinal$story %in% noun_story), c("subid", "story", "trial", "condition", "key", "answer", "accuracy")]
