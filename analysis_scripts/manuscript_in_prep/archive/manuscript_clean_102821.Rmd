...
``` {r}
setwd("/Users/jojohu/Documents/Qlab/manuscript_jndd")

allData <- read.csv("/Users/jojohu/Documents/Qlab/manuscript_jndd/allSLData.csv")
...
rawStd <- read.csv("/Users/jojohu/Documents/Qlab/manuscript_jndd/rsr_raw_std_table.csv")
...
```{r}
d_prime <- read.csv("/Users/jojohu/Documents/Qlab/manuscript_jndd/d_prime.csv")
...
## Preprocess by trial RT data, remove the RT Slope outliers
```{r, include = F}
# rt_trial <- read.csv("/Users/jojohu/Documents/Qlab/manuscript_jndd/rt_trial.csv")
...
# write.csv(rt_trial_scaled,"/Users/jojohu/Documents/Qlab/manuscript_development/rt_target_trial_td_asd.csv")
```
...
ggsave("composite_age_median.png",
        bg="transparent",width = 30, height = 19, units = "cm")
```
...
# ggsave("/Users/jojohu/Documents/Qlab/iascl_2019_2020/iascl_2021/composite_ssl_gjt.png",
#         bg="transparent", width = 15, height = 15, units = "cm")
```
...
compositeAgeL %>%
  ...
  arrange(domain) %>%
  mutate(Domain = dplyr::recode(domain, composite_ling = "Lingusitic", composite_nonling = "Nonlinguistic")) %>%
  ggplot(aes(x = rsr_std, y = composite_score, shape = Domain)) +
    scale_color_manual(values = c("black", "black"), guide="none") +
    scale_shape_manual(values = c(19, 1)) +
    geom_point(size = 4, position = pd, stroke = 1, color = "black", alpha = 0.7, show.legend = T) +
        labs(x = "Sentence Recall Standard Score",  # Change x-axis label
             y = "SL Composite Score") +
    theme(plot.title = element_text(hjust = 0.35)) +
    ...
    labs(color= "group") +
    geom_smooth(data = compositeAge[which(compositeAge$group == "ASD"),],
                aes(x = rsr_std, y = composite_ling),
                method=lm, se=F, inherit.aes = F, color = "black", linetype = 1, show.legend = F) +
     geom_smooth(data = compositeAge[which(compositeAge$group == "ASD"),],
                aes(x = rsr_std, y = composite_nonling),
                method=lm, se=F, inherit.aes = F, color = "black", linetype = 2, show.legend = F)

ggsave("/Users/jojohu/Documents/Qlab/manuscript_jndd/rsr_std_composite_ling_nonling.png",
        bg="transparent", width = 15, height = 15, units = "cm")
```
...
compositeAge %>%
  ...
  ggplot(aes(x = rsr_raw, y = composite_ling,color = group)) +
  ...
  geom_smooth(data = subset(compositeAge, group == "ASD"),
              aes(x = rsr_raw, y = composite_ling),
              method=lm, se=F, show.legend = F, inherit.aes = F, color = "black")
  
ggsave("/Users/jojohu/Documents/Qlab/manuscript_jndd/rsr_composite_ling_fixed.png",
...
```{r}
ggsave("/Users/jojohu/Documents/Qlab/manuscript_jndd/lang_level_composite_ling.png",
