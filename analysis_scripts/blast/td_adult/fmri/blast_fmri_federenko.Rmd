...
# IMPORTANT: Change dataset
```{r}
allTD <- read.csv("/Users/jojohu/Downloads/Blast_mri_cumulative_data.xlsx - TD.csv", stringsAsFactors = F)
...
langASLTD <- 
  langASL %>%
  filter(str_detect(part_id, "blast_c")) %>%
  filter(part_id %in% unique(allTD$ID))

langASLAd <-
  langASL %>%
  filter(str_detect(part_id, "blast_a")) 

langASL <- 
  rbind(langASLTD, langASLAd) %>%
  distinct(.)
...
# Add lang loc group constrained parcel hemisphere
langASL_long <- 
  langASL_long %>%
  mutate(hemi = case_when(
    # group == "adult" & parcel == "langloc_gcp3" ~ "Right Hemisphere",
    parcel == "langloc_gcp3" ~ "Right Hemisphere",
    parcel == "langloc_gcp4" ~ "Right Hemisphere",
    TRUE ~ "Left Hemisphere"
  ))


langASL_long %>%
  dplyr::select(parcel, hemi) %>%
  distinct(.)
```



# Remove ASL motion outliers or ASL runs with negative onsets
```{r}
# From blast_fmri_qc.Rmd script (temporary places for these saved files; will change after first level analyses are systemized)
negOnset <- read.csv("/Users/jojohu/Downloads/negOnset.csv", stringsAsFactors = F)
aslTP_outlier <- read.csv("/Users/jojohu/Downloads/aslTP_outlier.csv", stringsAsFactors = F)

negOnset$part_id <- paste("blast", str_extract(negOnset$part_id, "(?<=blast)\\S{1}"), str_extract(negOnset$part_id, "(?<=blast(a|c))\\S+"), sep = "_")
negOnset$run <- as.character(negOnset$run)

# Remove ASL runs with negative onsets
langASL_long <- 
  merge(langASL_long, negOnset[,-which(names(negOnset) %in% c("task"))], by = c("part_id", "run"), all.x = T) %>%
  filter(is.na(file_name)) %>%
  dplyr::select(-one_of("file_name"))

aslTP_outlier$part_id <- paste("blast", str_extract(aslTP_outlier$part_id, "(?<=blast)\\S{1}"), str_extract(aslTP_outlier$part_id, "(?<=blast(a|c))\\S+"), sep = "_")

aslTP_outlier <- 
  aslTP_outlier %>%
  mutate(stimuli = dplyr::recode(stimuli, syllable = "speech"),
         condition = dplyr::recode(condition, R = "rand", S = "str", B = "rest"))

aslTP_outlier %>%
  filter(asl_outlier == 1)

# Remove specific contrasts that are marked as motion outliers in their corresponding runs 
langASL_long <- 
  merge(langASL_long, aslTP_outlier[,-which(names(aslTP_outlier) %in% c("task", "stimcond"))], by = c("part_id", "stimuli", "condition"), all.x = T) %>%
  filter(asl_outlier == 0 | is.na(asl_outlier))

langASLID <- 
  langASL_long %>%
  dplyr::select(part_id, run, stimuli, condition) %>%
  distinct(.) %>%
  arrange(part_id, run)


danASL_long <- 
  merge(danASL_long, negOnset[,-which(names(negOnset) %in% c("task"))], by = c("part_id", "run"), all.x = T) %>%
  filter(is.na(file_name)) %>%
  dplyr::select(-one_of("file_name"))

# Remove specific contrasts that are marked as motion outliers in their corresponding runs 
danASL_long <- 
  merge(danASL_long, aslTP_outlier[,-which(names(aslTP_outlier) %in% c("task", "stimcond"))], by = c("part_id", "stimuli", "condition"), all.x = T) %>%
  filter(asl_outlier == 0 | is.na(asl_outlier))
...
# write.csv(langASLID, "/Users/jojohu/Downloads/langASLID.csv", row.names = F)
# write.csv(danID, "/Users/jojohu/Downloads/danID.csv", row.names = F)
```
...
# TO DO: Outlier removal for langloc with negative onsets
```{r, eval = F}
# langASL_long <-
#   langASL_long %>%
...
plotASL <- 
  function(hemiName, groupName, parcel_order, bar_outline, bar_color) {
  langASL_long %>%
    filter(!is.na(value)) %>%
    ...
    group_by(part_id, group, stimuli, condition, hemi, parcel, parcel_num) %>%
    dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
    summarySEwithin(., measurevar = "mean", betweenvars = c("group"),  withinvars = c("stimuli", "condition", "hemi", "parcel", "parcel_num"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = mean - ci,
           upper_ci = mean + ci) %>%
    ...
    ggplot(aes(x = group, y = mean, fill = parcel_num, pattern = factor(condition))) +
...
plotASL("Left Hemisphere", "adult", parcelLAd, "dark grey", c("#E69F00", "#009F73", "#D55E00", "#CC79A7"))
ggsave("/Users/jojohu/Downloads/asl_activation_adult_lh.png", height = 20, width = 30, unit= "cm")

plotASL("Left Hemisphere", "child", parcelLTD, "#A6CEE3", c("#E69F00", "#009F73", "#D55E00", "#CC79A7"))
ggsave("/Users/jojohu/Downloads/asl_activation_child_lh.png", height = 20, width = 30, unit= "cm")

plotASL("Right Hemisphere", "adult", parcelRAd, "dark grey", c("#F0E442", "#0072B1"))
ggsave("/Users/jojohu/Downloads/asl_activation_adult_rh.png", height = 20, width = 30, unit= "cm")

plotASL("Right Hemisphere", "child", parcelRTD, "#A6CEE3", c("#F0E442", "#0072B1"))
...
```{r}
plotASLByParcel <-
  function(hemiName, parcelNumber, stimName, bar_color) {
  langASL_long %>%
    filter(!is.na(value)) %>%
    mutate(group = dplyr::recode(group, adult = "Adult", child = "Child"), 
           condition = dplyr::recode(condition, str = "Structured", rand = "Random"),
           parcel = str_remove(parcel, "langloc_gcp")) %>% 
    group_by(part_id, group, stimuli, condition, hemi, parcel) %>%
    dplyr::summarise(value = mean(value, na.rm = T)) %>%
    spread(condition, value) %>%
    filter(!is.na(str)) %>%
    filter(!is.na(rand)) %>%
    gather(condition, value, -part_id, -group, -stimuli, -hemi, -parcel) %>%
    filter(hemi == hemiName & parcel == parcelNumber & stimuli == stimName) %>%
    group_by(part_id, group, condition) %>%
    summarySEwithin(., measurevar = "value", betweenvars = "group",  withinvars = "condition", idvar= c("part_id"), na.rm=T, conf.interval=.95) %>%
    mutate(lower_ci = value - ci,
           upper_ci = value + ci) %>%
    mutate(group = factor(group, levels = c("Adult", "Child")),
           condition = factor(condition, levels = c("Structured", "Random"))) %>%
    arrange(group, condition) %>%
    ggplot(aes(x = group, y = value, fill = condition)) +
    geom_bar(aes(color = group), stat = "identity", position = "dodge", size = 7) +
    geom_errorbar(aes(ymin=value-se, ymax=value+se),
                  position = position_dodge(0.9),
                  show.legend = FALSE,
                  width = .2) +
    scale_fill_manual(values = c(bar_color, alpha(bar_color, .5))) +
    scale_color_manual(values = c("dark grey", "#0072B1")) +
    guides(color = guide_legend(title = "Group", override.aes = list(fill = c("white", "white"), size = c(5, 5)))) +
    guides(fill = guide_legend(title = "Condition", override.aes = list(size = c(0, 0)))) +
    ylab("Mean Activation Level (Arbitrary Unit)") +
    theme_classic() +
    theme(axis.text.x = element_blank(),
          axis.ticks.x=element_blank(),
...
plotASLByParcel("Left Hemisphere", "1", "speech", "#E69F00")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel1.png", height = 20, width = 30, unit= "cm")

plotASLByParcel("Left Hemisphere", "2", "speech", "#009F73")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel2.png", height = 20, width = 30, unit= "cm")

plotASLByParcel("Left Hemisphere", "6", "speech", "#D55E00")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel6.png", height = 20, width = 30, unit= "cm")

plotASLByParcel("Left Hemisphere", "7", "speech", "#CC79A7")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel7.png", height = 20, width = 30, unit= "cm")

plotASLByParcel("Right Hemisphere", "3", "speech", "#F0E442")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel3.png", height = 20, width = 30, unit= "cm")

plotASLByParcel("Right Hemisphere", "4", "speech", "#A6CEE3")
ggsave("/Users/jojohu/Downloads/ssl_activation_parcel4.png", height = 20, width = 30, unit= "cm")
```

# Plot ASL Mean Activation (Stimuli * Condition)
```{r}
langASL_long %>%
  group_by(part_id, condition, stimuli) %>%
  dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
  summarySEwithin(., measurevar = "mean",  withinvars = c("condition", "stimuli"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
           upper_ci = mean + ci) %>%
  arrange(condition, stimuli) %>%
  mutate(condition = factor(condition, levels = c("str", "rand"))) %>%
  mutate(condition = dplyr::recode(condition, str = "Structured", rand = "Random"),
         stimuli = dplyr::recode(stimuli, speech = "Syllable", tone = "Tone")) %>% 
  ggplot(aes(x = condition, y = mean, fill = interaction(condition))) +
  ...
  geom_bar(aes(color = condition), stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), 
                position = position_dodge(0.9), 
                show.legend = FALSE, 
                width = .2) +
  facet_wrap("stimuli") +
  scale_fill_manual(values = c("black", alpha("black", .6)), guide = "none") +
  scale_color_manual(values = c("black", alpha("black", .6)), guide = "none") +
  # scale_pattern_manual(values=c("circle", "none"), 
  #                      guide = "legend") +
  ylab("Mean Activation Level (Arbitrary Unit)") +
  theme_classic() +
...
ggsave("/Users/jojohu/Downloads/asl_activation_stim_cond.png", height = 20, width = 40, unit= "cm")
```
...
langASL_long %>%
  filter(stimuli == "tone") %>%
  group_by(part_id, group, condition, stimuli) %>%
  dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
  summarySEwithin(., measurevar = "mean", betweenvars = c("group"),  withinvars = c("condition", "stimuli"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
           upper_ci = mean + ci) %>%
  arrange(group, condition, stimuli) %>%
  mutate(group = factor(group, levels = c("adult", "child")),
         condition = factor(condition, levels = c("str", "rand"))) %>%
  mutate(group = dplyr::recode(group, adult = "Adult", child = "Child"), 
         # stimuli = dplyr::recode(stimuli, speech = "Syllable", tone = "Tone"),
         condition = dplyr::recode(condition, str = "Structured", rand = "Random")) %>% 
  ggplot(aes(x = condition, y = mean, fill = interaction(condition, group))) +
  # ggplot(aes(x = condition, y = mean, fill = group, pattern = factor(condition))) +
  # geom_bar_pattern(stat = "identity", position = "dodge", color = "black") +
  geom_bar(aes(color = interaction(condition, group)), stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), 
                position = position_dodge(0.9), 
                show.legend = FALSE, 
                width = .2) +
  facet_grid(~group) +
  scale_fill_manual(values = c("dark grey", alpha("dark grey", .6), "#0072B1", alpha("#0072B1", .6)), guide = "none") +
  scale_color_manual(values = c("dark grey", alpha("dark grey", .6), "#0072B1", alpha("#0072B1", .6)), guide = "none") +
  # scale_pattern_manual(values=c("circle", "none"), 
  #                      guide = "legend") +
  ylab("Mean Activation Level (Arbitrary Unit)") +
  theme_classic() +
...
ggsave("/Users/jojohu/Downloads/asl_activation_group_cond_tone.png", height = 20, width = 40, unit= "cm")
```

# Plot ASL (Activation Across Runs)
```{r}
langASL_long %>%
  filter(stimuli == "speech") %>%
  mutate(run == as.factor(as.character(run))) %>%
  group_by(part_id, group, stimuli, condition, run) %>%
  dplyr::summarise(value = mean(value, na.rm = T)) %>%
  spread(condition, value) %>%
  # filter(!is.na(str)) %>%
  # filter(!is.na(rand)) %>%
  gather(condition, value, -part_id, -group, -stimuli, -run) %>%
  # filter(parcel == parcelNumber & stimuli == stimName) %>%
  group_by(part_id, group, condition, run) %>%
  summarySEwithin(., measurevar = "value", betweenvars = "group",  withinvars = c("condition", "stimuli", "run"), idvar= c("part_id"), na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = value - ci,
         upper_ci = value + ci) %>%
  mutate(condition = factor(condition, levels = c("str", "rand"))) %>%
  filter(!is.na(lower_ci)) %>%
  arrange(group, condition) %>%
  ggplot(aes(x = run, y = value, fill = interaction(condition, group))) +
  geom_bar(aes(color = group), stat = "identity", position = "dodge", size = 2) +
  geom_errorbar(aes(ymin=value-se, ymax=value+se),
                position = position_dodge(0.9),
                show.legend = FALSE,
                width = .2) +
  scale_fill_manual(values = c("dark grey", alpha("dark grey", .6), "#0072B1", alpha("#0072B1", .6)), guide = "none") +
  scale_color_manual(values = c("dark grey", "#0072B1")) +
  facet_wrap(~group+stimuli, strip.position = "top", ncol = 4) +
  guides(color = guide_legend(title = "Group", override.aes = list(fill = c("white", "white"), size = c(5, 5)))) +
  guides(fill = guide_legend(title = "Condition", override.aes = list(size = c(0, 0)))) +
  ylab("Mean Activation Level in the Language Network (Arbitrary Unit)") +
  theme_classic() 

ggsave("/Users/jojohu/Downloads/ssl_run_activation.png", width = 30, height = 18, units = "cm")   
```


# Plot ASL Mean Activation (Group * Condition * Hemi)
```{r}
langASL_long %>%
  filter(stimuli == "speech") %>%
  group_by(part_id, group, condition, stimuli, hemi) %>%
  dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
  summarySEwithin(., measurevar = "mean", betweenvars = c("group"),  withinvars = c("condition", "stimuli", "hemi"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
           upper_ci = mean + ci) %>%
  arrange(group, condition, stimuli) %>%
  mutate(group = factor(group, levels = c("adult", "child")),
...
ggsave("/Users/jojohu/Downloads/asl_activation_group_cond_hemi_speech.png", height = 20, width = 40, unit= "cm")
```
...
```{r}
langASL_long %>%
  group_by(part_id, stimuli, condition) %>%
  dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
  summarySEwithin(., measurevar = "mean",  withinvars = c("stimuli", "condition"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
         upper_ci = mean + ci) %>%
...
langASL_long %>%
  filter(stimuli == "speech") %>%
  dplyr::select(part_id, group, run) %>%
  distinct(.) %>%
...
asl_m1 <-
  lmer(value ~ group*stimuli*condition*hemi + perc_outlier + (1|parcel) + (1|part_id),  
       data = langASL_long,
...
# Broken down by stimuli across groups
asl_m1.2 <-
  lmer(value ~ group*condition + perc_outlier + (1+condition|part_id),  
...
asl_m1.2 <- summary(asl_m1.2)
asl_m1.2$coefficients <- round(asl_m1.2$coefficients, 2)
write.csv(as.data.frame(asl_m1.2$coefficients),"/Users/jojohu/Downloads/model_group_cond_speech_bold.csv")

# Broken down by stimuli across groups
asl_m1.3 <-
  lmer(value ~ condition + perc_outlier + (1|parcel) + (1|part_id),  
       data = langASL_long[which(langASL_long$stimuli == "tone"),],
      contrasts = list(condition = c(-0.5, 0.5),
                       hemi = c(-0.5, 0.5)),
       control = lmerControl(optimizer = "bobyqa"))
...
# Broken down by group within stimuli
asl_m1.4 <-
  lmer(value ~ condition + perc_outlier + (1|parcel) + (1|part_id),  
       data = langASL_long[which(langASL_long$stimuli == "speech" & langASL_long$group == "adult"),],
      contrasts = list(condition = c(-0.5, 0.5)),
       control = lmerControl(optimizer = "bobyqa"))
...
asl_m1.5 <-
  lmer(value ~ condition + perc_outlier + (1|parcel) + (1|part_id),  
       data = langASL_long[which(langASL_long$stimuli == "speech" & langASL_long$group == "child"),],
      contrasts = list(condition = c(-0.5, 0.5)),
       control = lmerControl(optimizer = "bobyqa"))
summary(asl_m1.5)
```
...
asl_m2Model <- summary(asl_m2)
asl_m2Model$coefficients <- round(asl_m2Model$coefficients, 2)
write.csv(as.data.frame(asl_m2Model$coefficients),"/Users/jojohu/Downloads/model_stim_cond_bold.csv")
...
tAll_marked <-
  ...
  mutate(roi = dplyr::recode(parcel_text, langloc_gcp1 = "Left Posterior Superior Temporal Gyrus", langloc_gcp2 = "Left Anterior Superior Temporal Gyrus",
                             langloc_gcp6 = "Left Inferior Frontal Gyrus", langloc_gcp7 = "left Middle Frontal Gyrus", 
                             langloc_gcp3 = "Right Posterior Superior Temporal Gyrus", langloc_gcp4 = "Right Anterior Superior Temporal Gyrus")) %>%
  mutate(stars = coalesce(three_star, two_star, one_star, marginal)) %>%
  select(-one_of("three_star", "two_star", "one_star", "marginal"))
  
write.csv(tAll_marked, "/Users/jojohu/Downloads/asl_t_test_str_rand.csv", row.names = F)
```
...
# Post-hoc paired t-tests with mean values in each group and in each condition
```{r}
# Group * Cond
mean_aslAll <-
  langASL_long %>%
...
t.test(mean_aslAdS$str, mean_aslAdS$rand, paired = T)
t.test(mean_aslTDS$str, mean_aslTDS$rand, paired = T)
...
langASL_long %>%
  ...
  group_by(part_id, group, stimuli, hemi) %>%
  dplyr::summarise(mean = mean(percent_change, na.rm = T)) %>%
  summarySEwithin(., measurevar = "mean",  betweenvars = c("group"), withinvars = c("stimuli", "hemi"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
         upper_ci = mean + ci) %>%
...
paslS_m1 <-
  lmer(percent_change ~ group + (1+hemi|part_id),  
       data = percentASLS,
      contrasts = list(group = c(-0.5, 0.5)))

summary(paslS_m1)

t.test(percent_change~group, percentASLS)
```
...
dan %>%
  ...
  ggplot(aes(x = condition, y = mean, fill = interaction(condition, group))) +
  geom_bar(aes(color = group), stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), 
                position = position_dodge(0.9), 
                show.legend = FALSE, 
                width = .2) +
  # geom_bar_pattern(stat = "identity", position = "dodge", color = "black") +
  facet_grid(~stimuli+group) +
  # scale_pattern_manual(values=c("circle", "none"), 
...
ggsave("/Users/jojohu/Downloads/dan_activation.png", width = 35, height = 20, units = "cm")    
```
...
dan %>%
  ...
  group_by(part_id, group, stimuli, condition) %>%
  dplyr::summarise(mean = mean(value, na.rm = TRUE)) %>%
  summarySEwithin(., measurevar = "mean", betweenvars = c("group"),  withinvars = c("stimuli", "condition"), idvar= "part_id", na.rm=T, conf.interval=.95) %>%
  mutate(lower_ci = mean - ci,
         upper_ci = mean + ci) %>%
  arrange(stimuli, condition) %>%
  mutate(condition = factor(condition, levels = c("str", "rand"))) %>%
...
# Dorsal Attention Network (Activation across runs)
```{r}
dan %>%
  filter(stimuli == "speech") %>%
  mutate(run == as.factor(as.character(run))) %>%
  ...
  guides(color = guide_legend(title = "Group", override.aes = list(fill = c("white", "white"), size = c(5, 5)))) +
  guides(fill = guide_legend(title = "Condition", override.aes = list(size = c(0, 0)))) +
  ylab("Mean Activation Level in the Dorsal Attention Network (Arbitrary Unit)") +
  theme_classic() 

ggsave("/Users/jojohu/Downloads/dan_run_activation.png", width = 30, height = 18, units = "cm")   
```

# Dorsal Attention Network (Activation comparison within participants)
```{r}
dan %>%
  filter(stimuli == "speech") %>%
  filter(condition == "str") %>%
...
```{r}
dan_m1 <-
  lmer(value ~ group*stimuli*condition + perc_outlier + (1+stimuli+condition|part_id),  
       data = dan,
      contrasts = list(group = c(-0.5, 0.5), 
                       condition = c(-0.5, 0.5),
                       stimuli = c(-0.5, 0.5)),
...
dan_m2 <-
  lmer(value ~ group*condition + perc_outlier + (1|part_id),  
...
aslDiff_attDiff$condition <- "str-rand"
...
mean_dan_diff <- 
  ...
  group_by(part_id, group, stimuli, condition, mean_dan, parcel) %>%
  dplyr::summarise(mean_dan = mean(mean_dan, na.rm = T)) %>%
  spread(condition, mean_dan) %>%
  mutate(mean_dan = str-rand) %>%
  dplyr::select(-one_of("rand", "str", "<NA>"))
...
mean_asl_diff <- 
  ...
  filter(stimuli == "speech") %>%
  mutate(value = scale(value)) %>%
  group_by(part_id, group, stimuli, condition, parcel) %>%
  dplyr::summarise(value = mean(value, na.rm = T)) %>%
  spread(condition, value) %>%
  mutate(value = str-rand) %>%
  dplyr::select(-one_of("rand", "str", "<NA>"))

aslDiff_attDiff <- merge(mean_asl_diff, mean_dan_diff, by = c("part_id", "group", "stimuli", "parcel"), all.x = T)
...
plot_danCorr <-
  function(df, groupName, colorPoint) {
    ...
    geom_point(size = 6, shape = 1, position = position_dodge(.2), stroke = 3, alpha = 0.3) +
    labs(x = "Dorsal Attention Network Activation (Structured) (Arbitrary Unit)",  # Change x-axis label
        y = "Language Network Activation (Structured) (Arbitrary Unit)") +
   # facet_wrap("condition") +
    ...
    theme(plot.title = element_text(size = 22, face = "bold"),
      ...
      stat_cor(aes(x = mean_dan, y = value), method = "spearman", label.y = 1.5, label.x = -0.5,
...
ggsave("/Users/jojohu/Downloads/asl_dan_corr_child_sig.png", width = 40, height = 40, units = "cm")    
plot_danCorr(asl_att2, "adult", "dark grey")
ggsave("/Users/jojohu/Downloads/asl_dan_corr_adult_sig.png", width = 40, height = 40, units = "cm")    
...
ggsave("/Users/jojohu/Downloads/asl_diff_dan_diff_corr_adult_sig.png", width = 40, height = 40, units = "cm")    
plot_danCorr(aslDiff_attDiff, "child", "#A6CEE3")
ggsave("/Users/jojohu/Downloads/asl_diff_dan_diff_corr_child_sig.png", width = 40, height = 40, units = "cm") 
```
...
mean_diff_asl_dan <- 
  aslDiff_attDiff %>%
    filter(group == "adult" & stimuli == "speech" & (condition == "str-rand")) %>%
    filter(parcel %in% sigParcelAd$parcel_text) %>%
    group_by(part_id, group, stimuli, condition, mean_dan) %>%
    dplyr::summarise(value = mean(value, na.rm = T))

cor.test(mean_diff_asl_dan$mean_dan, mean_diff_asl_dan$value, method = "spearman")

mean_diff_asl_dan <- 
  aslDiff_attDiff %>%
    filter(group == "child" & stimuli == "speech" & (condition == "str-rand")) %>%
    filter(parcel %in% sigParcelTD$parcel_text) %>%
    group_by(part_id, group, stimuli, condition, mean_dan) %>%
    dplyr::summarise(value = mean(value, na.rm = T))
...
```{r}
 aslDiff_attDiff %>%
    filter(stimuli == "speech" & condition == "str-rand") %>%
    # filter(parcel %in% sigParcelAd$parcel_text) %>%
    group_by(part_id, group, stimuli, condition, mean_dan) %>%
    dplyr::summarise(value = mean(value, na.rm = T)) %>%
    mutate(group = dplyr::recode(group, adult = "Adult", child = "Child")) %>%
    ggplot(aes(x = mean_dan, y = value, color = group)) +
    scale_color_manual(values = c("dark grey", "#0072B1")) +
    geom_point(size = 6, shape = 1, position = position_dodge(.2), stroke = 3, alpha = 0.3, show.legend = F) +
    labs(x = "Dorsal Attention Network Activation Difference Between Structured and Random Syllable Stream (Arbitrary Unit)",  # Change x-axis label
        y = "Language Network Activation Difference Between Structured and Random Syllable Streams (Arbitrary Unit)") +
   # facet_wrap("condition") +
    guides(color = guide_legend(title="Group", color = "black", override.aes = list(fill = "white"))) +
    # facet_grid(c("stimuli", "condition")) +
    theme(plot.title = element_text(size = 22, face = "bold"),
      ...
      geom_smooth(size = 3, method=lm, se=FALSE, show.legend = T) +
      stat_cor(aes(x = value, y = mean_dan), method = "pearson", label.y = 1, label.x = -1.7,
                inherit.aes = F, color = "black", size = 10) 

ggsave("/Users/jojohu/Downloads/asl_diff_dan_diff_corr_sig.png", width = 40, height = 40, units = "cm")    
```
...
aslDiff_attDiffC <- 
  ...
  dplyr::summarise(value = mean(value, na.rm = T)) %>%
  filter(group == "child") %>%
  filter(!is.na(mean_dan) | !is.na(value))
...
cor.test(aslDiff_attDiffA$mean_dan, aslDiff_attDiffA$value)
cor.test(aslDiff_attDiffC$mean_dan, aslDiff_attDiffC$value)

# Cocor compare correlations
library(cocor)
cocor.indep.groups(r1.jk=+0.4651241, r2.hm=+0.3742941, n1=27, n2=17, alpha=0.05, conf.level=0.95, null.value=0)
```
...
asl_att2 %>%
  ...
  geom_point(size = 4, shape = 1, position = position_dodge(0.2), stroke = 1.7,) +
...
# MVPA analysis
# Read in MVPA data (intact vs. rest in langloc)
```{r}
# Lang loc: Intact vs. rest
mvpaA <-
  read.csv("/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/indiv_langloc_output/asl/adult/results_mvpa_i_v_r/rval_in_parcel.csv", stringsAsFactors = F)

mvpaC <-
  read.csv("/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/indiv_langloc_output/asl/child/results_mvpa_i_v_r/rval_in_parcel.csv", stringsAsFactors = F)

mvpaIR <- 
  dplyr::bind_rows(mvpaA, mvpaC) %>%
  gather(key = parcel, value = value,  -ID, -ASL, -LANG_LOC) %>%
  mutate(part_id = ID,
         run = str_extract(ASL, "(?<=_run_)[[:digit:]]"),
         stimuli = str_extract(ASL, "(speech|tone)"),
         condition = str_extract(ASL, "(str|rand)"),
         parcel = str_extract(parcel, "gcp\\S+"),
         group = ifelse(str_detect(ID, "(_a)"), "adult", ifelse(str_detect(ID, "(_c)"), "child", NA)))
...
mvpa_m1A <-
  lmer(value ~ condition + (1+condition|part_id),  
       data = mvpaIR_speech[which(mvpaIR_speech$group == "adult" & mvpaIR_speech$parcel == "gcp7"),],
      contrasts = list(condition = c(-0.5, 0.5)),
       control = lmerControl(optimizer = "bobyqa"))
...
mvpa_m1C <-
  lmer(value ~ condition + (1+condition|part_id),  
       data = mvpaIR_speech[which(mvpaIR_speech$group == "child" & mvpaIR_speech$parcel == "gcp6"),],
      contrasts = list(condition = c(-0.5, 0.5)),
       control = lmerControl(optimizer = "bobyqa"))
...
mean_mvpaIR <-
  mvpaIR_speech %>%
  group_by(ID, group, stimuli, condition, parcel) %>%
  filter(parcel == "gcp1") %>%
  filter(group == "child") %>%
  dplyr::summarise(value = mean(value, na.rm = T))

t.test(mean_mvpaIR$value ~ mean_mvpaIR$condition)
```
