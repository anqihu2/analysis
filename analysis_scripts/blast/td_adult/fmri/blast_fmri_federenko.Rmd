...
langASL_long %>%
  ...
  # scale_pattern_manual(values = c("circle", "none")) +
  # scale_pattern_spacing_discrete(range = c(0.025, 0.05)) +
  # guides(pattern = guide_legend(title = "Condition", color = "black", override.aes = list(fill = c("white", "white")))) +
  theme +
    theme(axis.ticks.x=element_blank(),
          axis.title.x = element_blank())
    

ggsave("/Users/jojohu/Downloads/asl_activation_group_cond_speech_tone.png", height = 20, width = 35, unit= "cm")
```
...
# Broken down by stimuli
asl_m1.2 <-
  lmer(value ~ group*condition*hemi + perc_outlier + (1|parcel) + (1|part_id),  
...
plot_danCorr <-
  function(df, groupName, colorPoint) {
    df %>%
    filter(group == groupName & stimuli == "speech" & (condition == "str-rand")) %>%
