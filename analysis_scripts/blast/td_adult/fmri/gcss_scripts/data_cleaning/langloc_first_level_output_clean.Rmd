...
file.copy(as.character(aliceFile$aliceFile), aliceFile$new_path, overwrite = F)
...
# Clean and move TD children files
```{r}
library(stringr)
...
aliceFileAd <- aliceFileAd[sample(nrow(aliceFileAd), 27), ]
...
# Clean and move ASD children files
```{r}
library(stringr)
...
aliceFile <- 
  aliceFile %>%
  filter(format_id %in% unique(asdID$ID)) %>%
  distinct(.)
...
aliceFile$new_path <- paste0("/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/nilearn_pipeline/child/asd/",
                             aliceFile$basename)

aliceFile$new_name <- paste0("/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/nilearn_pipeline/child/asd/",
                             aliceFile$rename_id)

aliceFileASD <- aliceFile
...
write.csv(aliceFile, 
          "/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/nilearn_pipeline/renamed_id_asd.csv", row.names = F)

my_output <- capture.output(write.table(aliceFile[, "bare_id"], sep = " ", col.names = F, row.names = F, quote = F, eol = "\n"))
writeBin(paste(my_output, collapse = "\n"),  "/Volumes/data/projects/blast/data/mri/imaging/scott_gcss_lpsa/data/nilearn_pipeline/langloc_asd.txt")
```
