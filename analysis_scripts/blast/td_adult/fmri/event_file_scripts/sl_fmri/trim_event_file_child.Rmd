...
## Change event file names for those who have correct number of runs preprocessed and have all four runs
```{r}
changeRun1 <-
  changeRun %>%
  filter(bids_count == 4)

splitOrder <- as.data.frame(do.call(rbind, str_split(changeRun1$run_order, ", ")))

colnames(splitOrder) <- c("run1_name", "run2_name", "run3_name", "run4_name")

changeRun1 <- cbind(changeRun1, splitOrder)

changeRun1[which(changeRun1$Run == 1), "correctRun"] <- as.character(changeRun1[which(changeRun1$Run == 1),]$run1_name)
changeRun1[which(changeRun1$Run == 2), "correctRun"] <- as.character(changeRun1[which(changeRun1$Run == 2),]$run2_name)
changeRun1[which(changeRun1$Run == 3), "correctRun"] <- as.character(changeRun1[which(changeRun1$Run == 3),]$run3_name)
changeRun1[which(changeRun1$Run == 4), "correctRun"] <- as.character(changeRun1[which(changeRun1$Run == 4),]$run4_name)

changeRun1$renameFile <- str_replace(changeRun1$archive, "(?<=run-0)\\S{1}", changeRun1$correctRun)
...
## Change event file names for those who have correct number of runs preprocessed but have missing runs
```{r}
changeRun2 <-
  changeRun %>%
  filter(bids_count != 4)

runList <- unique(changeRun2[,c("PartID", "task", "bids_run")])

changeRun2$correctRun <- unlist(str_split(runList$bids_run, ", "))

changeRun2$renameFile <- str_replace(changeRun2$archive, "(?<=run-0)\\S{1}", changeRun2$correctRun)

changeRun2$renameFile <- str_remove(changeRun2$renameFile, "/original_event_files")

changeRun2 <-
  changeRun2 %>%
  filter(!is.na(eventFile))

print(changeRun2)

file.rename(changeRun2$archive, changeRun2$renameFile)
```
