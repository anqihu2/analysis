...
```{r}
spoli_rsr <- read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/RSR Online Home Scores  - spoli scores (rsr_rsr1).csv", stringsAsFactors = F)[,c(1:4)]
blast_rsr <- read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/RSR Online Home Scores  - blast scores (rsr_rsr1).csv",  stringsAsFactors = F)[,c(1:4)]
...
gender_wide <- as.data.frame(gender_wide)

# Something is wrong with the format of the data frame. chisq.test cannot be used. Can use this as a temp solution: https://www.socscistatistics.com/tests/chisquare2/default2.aspx
...
# SSL & TSL only data demographics before RT outlier removal
```{r}
allData %>%
  filter(!is.na(tsl_acc) | !is.na(ssl_acc)) %>%
  group_by(group) %>%
  dplyr::summarise(n = n(), mean_age = mean(age_at_web_year), max = max(age_at_web_year), min = min(age_at_web_year))

allData %>%
  filter(!is.na(tsl_acc) & !is.na(ssl_acc)) %>%
  dplyr::summarise(completed_ssl_tsl = n())
...
hit_trial_mean <-
  ...
  dplyr::select(group, task, mean_hit_trial, sd, lower_ci, upper_ci) %>%
...
m2_hit_trial <-
lmer(hit_trial_n ~ group*domain + (1|id), data = hit_trialNAd[which(hit_trialNAd$modality == "auditory"),],
       contrasts = list(group = c(0.5,-0.5),
                        domain = c(0.5, -0.5)),
     control=lmerControl(optimizer = "bobyqa"))
summary(m2_hit_trial)

hitsTD <- subset(hit_trialNAd, group == "TD")
hitsA <- subset(hit_trialNAd, group == "adult")

# Post-hoc t-tests for number of hits in each task
t.test(hitsA[which(hitsA$task == "lsl"),]$hit_trial_n, hitsTD[which(hitsTD$task == "lsl"),]$hit_trial_n)
t.test(hitsA[which(hitsA$task == "ssl"),]$hit_trial_n, hitsTD[which(hitsTD$task == "ssl"),]$hit_trial_n)
t.test(hitsA[which(hitsA$task == "vsl"),]$hit_trial_n, hitsTD[which(hitsTD$task == "vsl"),]$hit_trial_n)
t.test(hitsA[which(hitsA$task == "tsl"),]$hit_trial_n, hitsTD[which(hitsTD$task == "tsl"),]$hit_trial_n)
```
...
initial_rt <- 
  rt_trial_scaled %>%
  dplyr::arrange(id, group, task, trial, targ_index) %>%
  dplyr::group_by(id, task) %>%
  dplyr::filter(row_number()==c(1:4)) %>%
...
lmer_slope_m1 <-
  lmer(reaction_time ~ group*targ_index*domain*modality + hit_trial_n + rt_intercept + (1+domain+modality|id),
...
lmer_slope_m2 <-
  lmer(reaction_time ~ group*targ_index*domain + hit_trial_n + rt_intercept + (1+domain|id),
    # + (0 + (modality + domain) | id),
    data = rt_trial_scaled[which(rt_trial_scaled$modality == "auditory"),],
    contrasts = list(
      group = c(-0.5, 0.5),
      domain = c(-0.5, 0.5)
    ),
...
lmer_slope_m2.1 <-
  lmer(reaction_time ~ group*targ_index + hit_trial_n + rt_intercept + (1|id),
    # + (0 + (modality + domain) | id),
    data = rt_trial_scaled[which(rt_trial_scaled$modality == "auditory" & rt_trial_scaled$domain == "ling"),],
    contrasts = list(
      group = c(-0.5, 0.5)
    ),
    control=lmerControl(optimizer = "bobyqa")
  )
summary(lmer_slope_m2.1)

lmer_slope_m2.2 <-
  lmer(reaction_time ~ group*targ_index + hit_trial_n + rt_intercept + (1|id),
    # + (0 + (modality + domain) | id),
    data = rt_trial_scaled[which(rt_trial_scaled$modality == "auditory" & rt_trial_scaled$domain == "nonling"),],
    contrasts = list(
      group = c(-0.5, 0.5)
    ),
    control=lmerControl(optimizer = "bobyqa")
  )
summary(lmer_slope_m2.2)
...
lmer_slope_m3 <-
  lmer(scaled_rt ~ targ_index*domain*modality + hit_trial_n + sl_aprime + (1+domain+modality|id),
    # + (0 + (modality + domain) | id),
...
sapply(intersect(colnames(blast_adult_wide),colnames(allData))[str_detect(intersect(colnames(blast_adult_wide),colnames(allData)), "slope")],
                 function(x) t_test_multi_pair_slope(allData[,x], blast_adult_wide[,x]))
...
target <- 
  dp %>%
  filter(!is.na(part_id)) %>%
  select(part_id, task, target, targ) %>%
  distinct(.) %>%
  mutate(target = coalesce(target, targ)) %>%
  select(part_id, task, target)
...
triplet_list <- rbind(adult_triplet, child_triplet)
...
logacc_dom_m2 <-
  glmer(corr~group*domain + (1+domain|subj), data = acc_trial[which(acc_trial$modality == "auditory"),],
    family = binomial,
    contrasts = list(domain = c(-0.5, 0.5), group = c(-0.5,0.5)),
    control=glmerControl(optimizer = "bobyqa"))
...
logacc_dom_m3 <-
  glmer(corr~group*domain*hit_trial_n*modality + (1+domain+modality|subj), data = acc_trial,
    family = binomial,
...
write.csv(as.data.frame(accModel), "/Users/jojohu/Documents/Qlab/manuscript_development/accModel.csv")
```


### Attention Effect on Test Phase by Item
```{r}
logacc_item <-
  glmer(corr~group*domain*modality*trial_type + (1+domain+modality|subj), data = acc_trial,
    family = binomial,
    contrasts = list(domain = c(-0.5, 0.5), modality = c(-0.5, 0.5), 
                     group = c(-0.5,0.5), trial_type = c(-0.5, 0.5)),
    control=glmerControl(optimizer = "bobyqa"))
...
logacc_item2 <-
  glmer(corr~group*domain*trial_type + (1+domain+trial_type|subj), data = acc_trial[which(acc_trial$modality == "auditory"),],
    family = binomial,
...
medianFunc <-
function(groupName, domainName) {
  temp <- 
    hit_trialNAd %>%
    group_by(id, group, domain) %>% 
...
adultLingHit <- medianFunc("adult", "ling")
adultNonlingHit <- medianFunc("adult", "nonling")
tdLingHit <- medianFunc("TD", "ling")
tdNonlingHit <- medianFunc("TD", "nonling")

hit_median <- unique(rbind(adultLingHit, adultNonlingHit, tdLingHit, tdNonlingHit))

acc_trial <- merge(acc_trial, hit_median, by.x = c("subj", "group", "domain"), by.y = c("id", "group", "domain"), all.x = T)

# Check whether the adults with high hit rate results in worse non_target trial accuracy; adults pay too much attention to the target triplet compared to children
target_acc <-
  acc_trial %>%
  # filter(trial_type == "nontarget") %>%
  group_by(subj, group, task, trial_type, hit_group) %>%
  mutate(corr = as.numeric(as.character(corr))) %>%
  dplyr::summarize(mean_acc_id = mean(corr, na.rm = T), hit_rate = mean(hit_trial_n, na.rm = T) , n = sum(!is.na(corr))) 


targAccW <- cast(target_acc, subj + group + task + hit_rate + hit_group ~ trial_type, value = "mean_acc_id") 

targAccW$targ_nontarg <- targAccW$target - targAccW$nontarget
 
targAccW %>%
  # filter(trial_type == "nontarget") %>%
  group_by(group, task, hit_group) %>%
  dplyr::summarize(targ_nontarg = mean(targ_nontarg, na.rm = T), hit_rate = mean(hit_rate, na.rm = T) , n = length(unique(subj)))

library(ggplot2)

targAccW %>%
  filter(task == "ssl") %>%
  filter(target > 0.5 | nontarget > 0.5) %>%
  ggplot(aes(x = targ_nontarg, y = hit_rate, color = group)) +
  geom_point() +
...
targAccW %>%
  filter(task == "ssl") %>%
  filter(target > 0.5 | nontarget > 0.5) %>%
  ggplot(aes(x = target, y = hit_rate, color = group)) +
  geom_point() +
  facet_grid(~task + group)
...
targAccW %>%
  filter(task == "ssl") %>%
  filter(target > 0.5 | nontarget > 0.5) %>%
  ggplot(aes(x = nontarget, y = hit_rate, color = group)) +
  geom_point() +
  facet_grid(~task + group)
```
...
acc_trial %>%
  ...
  #   data = data.frame(Group = "Adult", Task = "Syllable"), 
  #   aes(x = 1, y = 0.725, label = paste0("**"))
  # ) +
  # geom_hline(yintercept = 0.5, size = 0.5, color = "black") +
  #  geom_text(size = 10, color = "black",
  #   data = data.frame(Group = "Child", Task = "Syllable"), 
  #   aes(x = 1, y = 0.722, label = paste0("***"))
  # )
 
  

# ggsave("/Users/jojohu/Documents/Qlab/manuscript_development/acc_by_trial_type.png",
#        bg="transparent",width = 30, height = 15, units = "cm")

ggsave("/Users/jojohu/Downloads/acc_by_trial_type_adult.png",
       bg="transparent",width = 30, height = 15, units = "cm")
...
logacc_hit_adult <-
  glmer(corr~domain*modality*hit_count + (1+domain|subj), data = acc_trial_adult,
    family = binomial,
    contrasts = list(domain = c(-0.5, 0.5), modality = c(-0.5, 0.5)),
    control=glmerControl(optimizer = "bobyqa"))
...
logacc_hit_td <-
  glmer(corr~domain*modality*hit_count + (1+domain|subj), data = acc_trial_td,
    family = binomial,
    contrasts = list(domain = c(-0.5, 0.5), modality = c(-0.5, 0.5)),
    control=glmerControl(optimizer = "bobyqa"))
...
t.test(acc_hitAd[which(acc_hitAd$domain == "ling" & acc_hitAd$hit_group == "higher_hit"),]$mean, 
       acc_hitAd[which(acc_hitAd$domain == "ling" & acc_hitAd$hit_group == "lower_hit"),]$mean)

t.test(acc_hitAd[which(acc_hitAd$domain == "nonling" & acc_hitAd$hit_group == "higher_hit"),]$mean, 
       acc_hitAd[which(acc_hitAd$domain == "nonling" & acc_hitAd$hit_group == "lower_hit"),]$mean)
...
t.test(acc_hitTD[which(acc_hitTD$domain == "ling" & acc_hitTD$hit_group == "higher_hit"),]$mean, 
       acc_hitTD[which(acc_hitTD$domain == "ling" & acc_hitTD$hit_group == "lower_hit"),]$mean)

t.test(acc_hitTD[which(acc_hitTD$domain == "nonling" & acc_hitTD$hit_group == "higher_hit"),]$mean, 
       acc_hitTD[which(acc_hitTD$domain == "nonling" & acc_hitTD$hit_group == "lower_hit"),]$mean)

# Attention facilitation seems to be nonlinear as a function of attention. Adults with higher hit rate performed similarly with those with lower hit rate, while children with higher hit rate outperformed those with lower hit rate in Syllable SL
acc_hit_mean <-
  acc_trial %>%
  filter(!is.na(hit_group)) %>%
  mutate(corr = as.numeric(as.character(corr))) %>%
...
t.test(acc_hit_mean[which(acc_hit_mean$group == "TD" & acc_hit_mean$task == "ssl" & 
                            acc_hit_mean$hit_group == "higher_hit"),]$mean, 
       acc_hit_mean[which(acc_hit_mean$group == "TD" & acc_hit_mean$task == "ssl" & 
                            acc_hit_mean$hit_group == "lower_hit"),]$mean)

t.test(acc_hit_mean[which(acc_hit_mean$group == "adult" & acc_hit_mean$task == "ssl" & 
                            acc_hit_mean$hit_group == "higher_hit"),]$mean, 
       acc_hit_mean[which(acc_hit_mean$group == "adult" & acc_hit_mean$task == "ssl" & 
                            acc_hit_mean$hit_group == "lower_hit"),]$mean)
```
...
## SL Group Comparison Accuracy: Post-hoc T-test Adults vs. TD
```{r}
t_test_multi_pair <- 
  function(x,y){
  test <- t.test(x, y)
...
getDescripStat <- function(group, col) {
  compositeAge %>%
      ...
      mutate(se = sd / sqrt(n),
             lower_ci = mean - qt(1 - (0.05 / 2), n - 1) * se,
             upper_ci = mean + qt(1 - (0.05 / 2), n - 1) * se) %>%
      select(!!as.symbol(group), mean, sd, lower_ci, upper_ci) %>%
...
acc_within_stat %>%
  filter( variable == "Syllable" | variable == "Tone") %>%
  mutate(Group = factor(Group, levels=c("adult", "TD"))) %>%
  ...
  geom_text(aes(x = 2.3, y = 0.495, label = paste0("chance", "-level")), size = 4) +
  geom_text(aes(x = 1, y = 0.83, label = paste0("†")), size = 10, color = "black") +
  geom_text(aes(x = 2, y = 0.83, label = paste0("***")), size = 10, color = "black")
  # geom_text(aes(x = 1, y = 0.83, label = paste0("***")), size = 10, color = "black") +
...
ggsave("/Users/jojohu/Documents/Qlab/manuscript_development/acc_across_group.png",
...
slope_within_stat %>%
  filter(task == "Syllable" | task == "Tone") %>%
  mutate(Group = factor(Group, levels=c("adult", "TD"))) %>%
  mutate(Group = dplyr::recode(Group, "adult" = "Adult",  "TD" = "Children")) %>%
  ...
  geom_text(aes(x = 1, y = -5.5, label = paste0("***")), size = 10, color = "black")
...
ggsave(
    "/Users/jojohu/Documents/Qlab/manuscript_development/slope_across_group.png",
...
slope_within_stat_age %>%
  filter(task == "Syllable" | task == "Tone") %>%
  arrange(task, Group) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(task = factor(task, levels=c("Syllable", "Tone")),
         Group = factor(Group, levels=c("Younger TD", "Older TD", "Adult"))) %>% 
  dplyr::mutate(Group = dplyr::recode(Group, "Younger TD" = "Younger Child", "Older TD" = "Older Child")) %>%
  ...
  scale_y_reverse() +
  scale_color_manual(values = c("black", cbPalette[2],  cbPalette[3])) +
  scale_linetype_manual(values=c("dashed", "dotted", "twodash")) +
  # scale_y_reverse() +
  theme(plot.title = element_text(hjust = 0.35)) +
  theme(
...
ggsave(
    "/Users/jojohu/Documents/Qlab/manuscript_development/slope_across_age.png",
...
```{r}
slopeAgeTD <- 
  merge(slope_wideTD, demo_all[,c("part_id", "group", "age_at_web_year")], by.x = c("id"), by.y = c("part_id"), all.x = T)

slopeAgeAd <- slope_wideA
slopeAgeAd$group <- "adult"
slopeAgeAd$age_at_web_year <- 20

slopeAge <- rbind(slopeAgeAd, slopeAgeTD)

slopeAge %>%
  ggplot(aes(x = age_at_web_year, y = ssl)) +
  geom_point(
    aes(color = group),
    size = 4,
    position = pd,
...
hit_within_stat %>%
  ...
  theme(
   ...
   geom_text(aes(x = 1, y = 0.9, label = paste0("***")), size = 10, color = "black")
# +
#   geom_text(aes(x = 1, y = 43, label = paste0("***")), size = 10, color = "black") +
#   geom_text(aes(x = 2, y = 43, label = paste0("***")), size = 10, color = "black") +
#   geom_text(aes(x = 3, y = 43, label = paste0("***")), size = 10, color = "black") 



ggsave("/Users/jojohu/Documents/Qlab/manuscript_development/hit_across_task.png",
        bg="transparent", width = 15, height = 15, units = "cm")
...
## Plot Accuracy by Hit Rate and Domain Line Plot
```{r}
library(ggpubr)
...
acc_trial %>%
  ...
  dplyr::summarise(mean_corr = mean(corr, na.rm = T), 
                   mean_hit_rate = mean(hit_count, na.rm = T)) %>%
  # dplyr::select(part_id, task, sentence, mean, se, lower_ci, upper_ci, n) %>%
  mutate_if(is.numeric, round, 2) %>%
  arrange(group, domain) %>%
  ...
  ggplot(aes(x = mean_hit_rate, y = mean_corr, color = domain)) +
  facet_wrap(~group) +
  geom_point(aes(shape = type), size = 4, shape = 1, position = pd, stroke = 1.7) +
  # scale_fill_manual(values = c("white", "white")) +
  # scale_color_manual(values = c("dark grey", cbPalette[1])) +
  # guides(fill="none") +
...
ggsave("/Users/jojohu/Documents/Qlab/manuscript_development/acc_hit.png",
        bg="transparent", width = 30, height = 15, units = "cm")
```



## Plot Accuracy by Hit Rate and Domain Bar Plot
```{r}
library(ggpubr)
library(ggpattern)
...
ggsave("/Users/jojohu/Documents/Qlab/manuscript_development/acc_hit_age_group.png",
...
acc_trial %>%
  ...
  # mutate(task = factor(domain, levels=c("ling", "nonling"))) %>%
  mutate(group = dplyr::recode(group, "adult" = "Adult",  "TD" = "Children"),
        # domain = dplyr::recode(domain, "ling" = "Linguistic SL",  "nonling" = "Nonlinguistic SL"),
         hit_group = dplyr::recode(hit_group, "lower_hit" = "Lower Hit",  "higher_hit" = "Higher Hit")) %>%
  ggplot(aes(x = task, y = mean,
             ymin = mean - se, 
             ymax = mean + se,
             pattern = as.factor(hit_group),
             fill = group)) +
  facet_wrap(~group) +
  # scale_fill_manual(values = c("white", "white")) +
  scale_fill_manual(values = c("dark grey", cbPalette[1])) +
