...
for (i in targets) {
  ...
  two_preceding_trial_press <- lsl_all[i-2, "col1"]-lsl_all[i-2, "time"]
  ...
  # Anticipation, positive RT from preceding trial (only for participants with blanks of 1000 ms): 
  
  if (!is.na(lsl_all[i-1,]$col1 > 0) & (lsl_all[i-1,]$col1 > 0) & length(which(short_blank$par_id %in% lsl_all[i,]$par_id)) == 0){
    reaction_time <- append(reaction_time, (this_preceding_trial_press-1000))
    lsl_all[i,]$type <- "hit_before"
    type <- rbind(type, "hit_before")
    print("i - 1 ran")
  }
  # Anticipation, positive RT from preceding trial (only for participants with blanks of 200 ms): 
  else if (!is.na(lsl_all[i-2,]$col1 > 0) & (lsl_all[i-2,]$col1 > 0) & length(which(short_blank$par_id %in% lsl_all[i,]$par_id)) > 0){
    reaction_time <- append(reaction_time, (two_preceding_trial_press-1000))
    lsl_all[i,]$type <- "hit_before"
    type <- rbind(type, "hit_before")
    print("i - 2 ran")
  }
  ...
  # Delay, positive RT from following trial (only for participants with blanks of 1000 ms): 
  # else if ( !is.na(this_trial< lsl_all_end & lsl_all[i+1,]$press_time > 0) & (this_trial< lsl_all_end & lsl_all[i+1,]$press_time > 0)){
  else if (!is.na(lsl_all[i+1,]$col1 > 0) & (lsl_all[i+1,]$col1 > 0) & length(which(short_blank$par_id %in% lsl_all[i,]$par_id)) == 0){
  ...
  # Delay, positive RT from following trial (only for participants with blanks of 200 ms): 
  else if (!is.na(lsl_all[i+2,]$col1 > 0) & (lsl_all[i+2,]$col1 > 0) & length(which(short_blank$par_id %in% lsl_all[i,]$par_id)) > 0){
    reaction_time <- append(reaction_time, (1000+(lsl_all[i+2, "col1"]-lsl_all[i+2, "time"])))
    lsl_all[i,]$type <- "hit_after"
    type <- rbind(type, "hit_after")
    print("i + 2 ran.")
  }
...
write.csv(indiv_rts, paste0(output_path, "blast_online_adult_lsl_indiv_rts.csv"))
