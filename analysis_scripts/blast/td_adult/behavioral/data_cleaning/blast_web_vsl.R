...
vsl_entropy_key <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/entropy_keys/vsl_entropy_key.csv")
vsl_entropy_key <-
...
vsl_entropy_key_bal <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/balanced2afc_entropy_keys/vsl_bal_entropy.csv")
vsl_entropy_key_bal <-
...
for (id in all_ids) { 
  ...
  # blast_a_003 and blast_a_004 have different audio instruction, thus need to include "or" (|) statement
  exp_phase_end <- which(str_detect(this_id_vsl$stimulus, "vsl_audio/lsl_instr6.wav$|../../vsl_audio/vsl_instr_between.wav")) - 1
...
trial_order <- rep(1:32, times = length(unique(vsl_accuracy$subj)))
...
vsl_accuracy$trial_order <- trial_order
...
for (id in vsl_unb_id) {
  ...
  if (str_detect(id,"blast_a_003|blast_a_004")) {
    test_phase_end <- nrow(this_id_vsl) -1
  } else {
    test_phase_end <- which(str_detect(this_id_vsl$stimulus, "lsl_instr13.wav$|end_instr.wav")) - 1
  }
...
if (mean(test_phase_stim$current_id_correct_key) != 32) {
  ...
  print("Check subject IDs that do not have 32. They ARE supposed to be in the counterbalanced group.")
  stop()
}
...
vsl_accuracy_bal <- vsl_accuracy
accuracy_by_trial <- rbind(vsl_accuracy_unb[,c(1:5)], vsl_accuracy_bal[,c(1:5)])
accuracy_by_trial$key <- unlist(accuracy_by_trial$key)
...
vsl_test_phase <- dplyr::bind_rows(vsl_accuracy_unb, vsl_accuracy_bal)
vsl_test_phase$key <- unlist(vsl_test_phase$key)
write.csv(as.data.frame(vsl_test_phase), paste0(output_path, "acc_by_trial/blast_online_adult_vsl_test_phase.csv"))
...
write.csv(accuracy_by_trial, paste0(output_path, "acc_by_trial/blast_vsl_acc_by_trial.csv"))
