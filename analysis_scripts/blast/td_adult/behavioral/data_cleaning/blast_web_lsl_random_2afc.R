...
lsl_entropy_key <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/balanced2afc_entropy_keys/lsl_bal_entropy.csv")
lsl_entropy_key <-
...
trial_order <- rep(1:32, times = length(unique(lsl_accuracy_unb$subj)))


lsl_accuracy_unb$key <- keyv_unb
lsl_accuracy_unb$trial_order <- trial_order
...
# Read in the entropy key
lsl_entropy_key_unb <- read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/entropy_keys/lsl_entropy_key_randomized.csv")
...
lsl_test_phase <- dplyr::bind_rows(lsl_accuracy_unb, lsl_accuracy)
lsl_test_phase$key <- unlist(lsl_test_phase$key)
write.csv(as.data.frame(lsl_test_phase), paste0(output_path, "acc_by_trial/blast_online_adult_lsl_random_test_phase.csv"))
...
write.csv(lsl_entropy_by_triplet[,5:6], 
          paste0(output_path, "blast_online_adult_lsl_randomized_entropy.csv"))
...
acc_by_trial <- rbind(lsl_accuracy[,c(1:5)], lsl_accuracy_unb[,c(1:5)])
...
# Save mean accuracy  
write.csv(lsl_acc_table, paste0(output_path, "blast_online_adult_lsl_random_2afc_accuracies.csv"))

# Save accuracy by trial 
write.csv(acc_by_trial, paste0(output_path, "acc_by_trial/blast_lsl_random_acc_by_trial.csv"))
