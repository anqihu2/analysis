...
ssl_entropy_key <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/entropy_keys/ssl_entropy_key.csv")
ssl_entropy_key <-
...
# Read in the entropy key
ssl_entropy_key_bal <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/balanced2afc_entropy_keys/ssl_bal_entropy.csv")
ssl_entropy_key_bal <-
...
# Spoli_c_097 is a special case. Reaction time is not correctly collected in all the trials for this participant.
...
write.csv(indiv_rts, paste0(output_path, "blast_online_adult_ssl_indiv_rts.csv"))
...
trial_order <- rep(1:32, times = length(unique(ssl_accuracy$subj)))
...
ssl_accuracy$trial_order <- trial_order
...
ssl_accuracy_bal <- ssl_accuracy 
acc_by_trial <- rbind(ssl_accuracy_unb[,c("ans","subj","key","trial_order", "corr")], ssl_accuracy_bal[,c("ans","subj","key","trial_order", "corr")])
...
ssl_test_phase <- dplyr::bind_rows(ssl_accuracy_unb, ssl_accuracy_bal)
ssl_test_phase$key <- unlist(ssl_test_phase$key)
write.csv(as.data.frame(ssl_test_phase), paste0(output_path, "acc_by_trial/blast_online_adult_ssl_test_phase.csv"))
...
write.csv(ssl_entropy_by_triplet[,5:6], 
          paste0(output_path, "blast_online_adult_ssl_entropy.csv"))
#Save mean accuracy
write.csv(ssl_acc_table, paste0(output_path, "blast_online_adult_ssl_accuracies.csv"))

# Save accuracy by trial
write.csv(acc_by_trial, paste0(output_path, "acc_by_trial/blast_ssl_acc_by_trial.csv"))
