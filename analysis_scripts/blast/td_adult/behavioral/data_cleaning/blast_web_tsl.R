...
#  Adapted from mturk_tsl by An Nguyen and blast_web_tsl by Violet Kozloff
#  This script analyses reaction time, accuracy and entropy for TSL files from the online session of the BLAST experiment
#  ****************************************************************************
...
#Input path
input_path <- ("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/blast_adult/")
#Since June 11th 2019, the test phase of all SL tasks have been changed to a balanced design
#New answer keys should be used
input_path2 <- ("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/blast_adult/counterbalance_2afc_sl_data/")
...
tsl_entropy_key <-
  read.csv("//Users/jojohu/Documents/Qlab/blast_online_data/online_sl/entropy_keys/tsl_entropy_key.csv")
tsl_entropy_key <-
  (tsl_entropy_key[c("target_type",
                     "target_order",
...
# Read in the entropy key
tsl_entropy_key_bal <-
  read.csv("/Users/jojohu/Documents/Qlab/blast_online_data/online_sl/balanced2afc_entropy_keys/tsl_bal_entropy.csv")
tsl_entropy_key_bal <-
  (tsl_entropy_key_bal[c("target_type",
                     "answer_key",
                     "tone_target")])
...
write.csv(indiv_rts, paste0(output_path, "blast_online_adult_tsl_indiv_rts.csv"))

# write.csv(indiv_rts, paste0(output_path, "online_child_tsl_indiv_rts.csv"))
# TO DO: Add this for children

#Save rt by trial data
write.csv(exp_targets, paste0(output_path, "rt_by_trial/blast_tsl_rt_by_trial.csv"))
#End of reaction time analysis-----------------------------------------------------------------------


# Calculate and summarize individual accuracies ------------------------------------------------------------

#List the tsl files after June 11th, 2019 with counterbalanced target-foils in the test phase
#Extract the ids for balanced lsl
tsl_files_balanced <- list.files(path=input_path2, pattern="*tsl.csv")
tsl_bal_id <- str_extract(tsl_files_balanced, "\\S+(?=_tsl.csv)")
#List the files that are not counterbalanced which are collected before June 11th, 2019
tsl_files_unbal <- tsl_files[!(tsl_files %in% tsl_files_balanced)]
tsl_unb_id <- str_extract(tsl_files_unbal, "\\S+(?=_tsl.csv)")


#Extract the data for participants who did unbalanced design
tsl_unb <- tsl[which(tsl$part_id %in% unlist(tsl_unb_id)),]


#Extract the test phase
test_block <- tsl_unb[(tsl_unb$stimulus=="silent" & !tsl_unb$key_press==-1),]
...
trial_order <- rep(1:32, times = length(unique(tsl_accuracy$subj)))
...
tsl_accuracy$trial_order <- trial_order
...
corr <- NULL
for (i in seq(from = 1,
              to = length(tsl_accuracy$ans),
              by = 1)) {
  corr <-
    append(corr, as.numeric(tsl_accuracy[i, ]$ans == tsl_accuracy[i, ]$key))
}
...
for (id in tsl_unb_id) {
  loop_index = loop_index +1
  this_id_tsl <- tsl_unb[which(tsl_unb$part_id %in% id),]
  test_phase_index <- which(this_id_tsl$stimulus == "silent")
  test_phase_start<- test_phase_index[1]
  test_phase_end <- test_phase_index[length(test_phase_index)]
  test_phase_this_id <- this_id_tsl[test_phase_start:test_phase_end,]
  # Extract exposure phase
...
loop_index = 0 
test_phase_id <- unique(tsl_unb$part_id)
...
for (id in test_phase_id) {
  ...
  #Extract the rows that have the silent stimuli, tsl test phase starts and ends with silent
  row_white <- which(this_id_data$stimulus == "silent")
  ...
  #Remove the silent stimulus
  stim_test <- stim_test[which(stim_test$stimulus != "silent"),]
  #Add in trial number
  ...
  current_id_triplet$triplet <- paste0("tone", current_id_triplet$triplet)
  ...
  #First double all rows in tsl_entropy_keys
  doubled_entropy_key <- tsl_entropy_key[rep(1:nrow(tsl_entropy_key),1,each=2),]
  #Then make every other row's target triplet empty
  doubled_entropy_key[c(seq(2, dim(doubled_entropy_key)[1], by=2)), "tone_target"] <- NA
  ...
  check_every_other_row <- 
    as.numeric(current_id_triplet$triplet %in% doubled_entropy_key$tone_target)
  ...
  current_id_correct_key <- sum(as.numeric(current_id_key == tsl_entropy_key$target_order))
...
if (mean(test_phase_stim$current_id_correct_key) != 32) {
  ...
  print("Check subject IDs that do not have 32. They are not supposed to be in the uncounterbalanced group.")
  stop()
}
...
tsl_entropy_by_triplet$mean_entropy <-
  round(rowMeans(tsl_entropy_by_triplet[, 1:4], na.rm = FALSE, dims = 1), 3)
...
for (id in acc_id) {
  subj_corr <-
    append(subj_corr, round(sum(tsl_accuracy$corr[tsl_accuracy$subj == id]) /
                              32, digits = 3))
}

tsl_accuracy_unb <- tsl_accuracy

tsl_acc_table <- data.frame(acc_id, subj_corr)

#Why remove outliners here -jh?----------------------------------------------------
...
#List the tsl files after June 11th, 2019 with counterbalanced target-foils in the test phase
#Extract the ids for balanced tsl
tsl_files_balanced <- list.files(path=input_path2, pattern="*tsl.csv")
tsl_bal_id <- str_extract(tsl_files_balanced, "\\S+(?=_tsl.csv)")

#Extract the data for participants who did balanced design
tsl_bal <- tsl[which(tsl$part_id %in% unlist(tsl_bal_id)),]


#Extract the test phase
test_block <- tsl_bal[(tsl_bal$stimulus=="silent" & !tsl_bal$key_press==-1),]
...
language = list(2,1,2,1,2,1,1,2,2,1,2,2,1,2,1,1,2,1,2,1,2,1,2,1,2,2,1,1,2,2,1,1)
...
for (id in tsl_bal_id) {
  loop_index = loop_index +1
  this_id_tsl <- tsl_bal[which(tsl_bal$part_id %in% id),]
  test_phase_index <- which(this_id_tsl$stimulus == "silent")
  test_phase_start<- test_phase_index[1]
  test_phase_end <- test_phase_index[length(test_phase_index)]
  test_phase_this_id <- this_id_tsl[test_phase_start:test_phase_end,]
...
test_phase_stim <- list()
loop_index = 0 
test_phase_id <- unique(tsl_bal$part_id)
trial_index <- seq(from = 1, to = 192,  by = 1)
...
for (id in test_phase_id) {
  ...
  #Doing this will allow comparing 64 trials (targets + foils) with 32 trials (from entropy key)
  #First double all rows in tsl_entropy_keys
  doubled_entropy_key <- tsl_entropy_key_bal[rep(1:nrow(tsl_entropy_key),1,each=2),]
  #Then make every other row's target triplet empty
  doubled_entropy_key[c(seq(2, dim(doubled_entropy_key)[1], by=2)), "tone_target"] <- NA
  #Check the triplets of the current id's data against entropy key
  #Every other rowsin doubled_entropy_key shows the target triplet, so if every other row yeilds TRUE, then answer will be 1
  ...
  current_id_key <- gsub(0, 2, current_id_key)
  #Check current id's answer keys against the correct answer keys 
  current_id_correct_key <- sum(as.numeric(current_id_key == tsl_entropy_key_bal$answer_key))
  #Save the result into a list
  test_phase_stim[[loop_index]] <- data.frame(id, current_id_correct_key)
}
...
# Find the triplet type (each triplet gets coded with a value from A-D)
triplet_type <- rep(tsl_entropy_key_bal$target_type, times = length(unique(tsl_accuracy$subj)))
# Find the order for the triplet (the triplet either appeared first or second, with respect to the foil)
triplet_order <- rep(tsl_entropy_key_bal$answer_key, times = length(unique(tsl_accuracy$subj)))
# Find the syllable triplet (which three syllables make up the triplet)
tone_triplet <- rep(tsl_entropy_key_bal$tone_target, times = length(unique(tsl_accuracy$subj)))
...
#Caculate Entropy for each target type by group and by task
tsl_entropy_by_triplet_bal <- data.frame()

# tsl Entropy for each target type
for (i in 1:nrow(tsl_entropy_wide_bal)) {
  tsl_entropy_by_triplet_bal[i,"tsl_a_entropy"] <- Entropy(tsl_entropy_wide_bal[i,c("0_A","1_A")])
  tsl_entropy_by_triplet_bal[i,"tsl_b_entropy"] <- Entropy(tsl_entropy_wide_bal[i,c("0_B","1_B")])
  tsl_entropy_by_triplet_bal[i,"tsl_c_entropy"] <- Entropy(tsl_entropy_wide_bal[i,c("0_C","1_C")])
  tsl_entropy_by_triplet_bal[i,"tsl_d_entropy"] <- Entropy(tsl_entropy_wide_bal[i,c("0_D","1_D")])
  tsl_entropy_by_triplet_bal[i,"part_id"] <- tsl_entropy_wide_bal[i,c("subj")]
}


tsl_entropy_by_triplet_bal$mean_entropy <-
  round(rowMeans(tsl_entropy_by_triplet_bal[, 1:4], na.rm = FALSE, dims = 1), 3)
...
tsl_accuracy_bal <- tsl_accuracy
acc_by_trial <- rbind(tsl_accuracy_unb, tsl_accuracy_bal)
acc_by_trial$key <- unlist(acc_by_trial$key)

tsl_acc_table_bal <- data.frame(acc_id, subj_corr)


#End of balanced test phase accuracy analysis-----------------------------------------------------------------------------------------------------------------------------------

tsl_acc_table <- rbind(tsl_acc_table, tsl_acc_table_bal)

tsl_entropy_by_triplet <- rbind(tsl_entropy_by_triplet, tsl_entropy_by_triplet_bal)

tsl_entropy_wide <- rbind(tsl_entropy_wide, tsl_entropy_wide_bal)

#Save test phase item
tsl_test_phase <- rbind(tsl_accuracy_unb, tsl_accuracy_bal)
tsl_test_phase$key <- unlist(tsl_test_phase$key)
write.csv(as.data.frame(tsl_test_phase), paste0(output_path, "acc_by_trial/blast_online_adult_tsl_test_phase.csv"))
#Save raw entropy by triplet counts
write.csv(tsl_entropy_wide,
          paste0(output_path, "entropy_by_triplet/blast_tsl_entropy_raw_sums.csv"))
#Save entropy by triplet type
write.csv(tsl_entropy_by_triplet,
          paste0(output_path, "entropy_by_triplet/blast_tsl_entropy_by_triplet.csv"))
#Save mean entropy 
write.csv(tsl_entropy_by_triplet[,5:6], 
          paste0(output_path, "blast_online_adult_tsl_entropy.csv"))
#Save accuracy
write.csv(tsl_acc_table, paste0(output_path, "blast_online_adult_tsl_accuracies.csv"))

#Save accuracy by trial
write.csv(acc_by_trial[,c(1:5)], paste0(output_path, "acc_by_trial/blast_tsl_acc_by_trial.csv"))
