...
# logacc_dom <-
#   glmer(corr~group*domain*modality + (0+domain|subj) + (0+modality|subj) + (1|subj) + (1|trial_order), data = acc_trial,
#     family = binomial,
...
m1_hit_trial <-
lmer(hit_trial_n ~ group*domain*modality + (0+domain|id) + (0+modality|id), data = hit_trialN,
       contrasts = list(group = c(-1,1),
                        domain = c(-1,1),
                        modality = c(-1,1)))
...
lmerrt_scaled_m1 <-
  lmer(scaled_rt~group*targ_index*domain*modality + (1 | id),
       data = rt_trial_scaled,
       contrasts = list(group = c(-0.5,0.5),
                        modality = c(-0.5, 0.5),
                        domain = c(-0.5, 0.5))
...
lmerrt_scaled_m2 <-
  lmer(log_rt~group*targ_index*domain*modality + (1 + modality + domain | id) + (1|id),
       data = rt_trial_scaled,
       contrasts = list(group = c(-0.5,0.5),
                        modality = c(-0.5, 0.5),
                        domain = c(-0.5, 0.5)),
...
summary(lmerrt_scaled_m2)

anova(lmerrt_scaled_m1, lmerrt_scaled_m2)

rt_trial_scaled$scaled_rt_ten <- rt_trial_scaled$scaled_rt * 100

qqnorm(resid(lmerrt_scaled_m1))

plot(resid(lmerrt_scaled_m1), rt_trial_scaled$scaled_rt[!is.na(rt_trial_scaled$scaled_rt)])

scaledRTten_m1 <-
  lmer(scaled_rt_ten ~ group*targ_index*domain*modality + (1 + domain | id),
       data = rt_trial_scaled,
       contrasts = list(group = c(-0.5,0.5),
                        modality = c(-0.5, 0.5),
                        domain = c(-0.5, 0.5)),
...
summary(scaledRTten_m1)
```

```{r}
# Using ranked RT:
lmerrt_m3 <-
  lmer(normed_rankRT ~ group*targ_index*domain*modality + (1 + modality + domain | id),
       data = qnormRTall,
       contrasts = list(group = c(-0.5,0.5),
                        modality = c(-0.5, 0.5),
                        domain = c(-0.5, 0.5)))
...
```{r, include = F, eval = F}
# Adding hit trials into the model below yielded no significance
rt_trial_scaled$groupN <- ifelse(rt_trial_scaled$group == "ASD", 0, 1)
rt_trial_scaled$domainN <- ifelse(rt_trial_scaled$domain == "nonling", 0, 1)
rt_trial_scaled$modalityN <- ifelse(rt_trial_scaled$modality == "auditory", 0, 1)


m2 <-
  lmer(scaled_rt~groupN*targ_index*domainN*modalityN + (1 + (domainN)|id),
       data = rt_trial_scaled,
       # contrasts = list(group = c(-1,1),
       #                  modality = c(-1, 1),
       #                  domain = c(-1, 1)),
...
m3 <-
  lmer(reaction_time~groupN*targ_index*domainN*modalityN + (1|id),
       data = rt_trial_scaled,
       # contrasts = list(group = c(-1,1),
       #                  modality = c(-1, 1),
       #                  domain = c(-1, 1)),
...
print(lmerrt_trial_dom_scaled, correlation=TRUE)


logRT_m1 <-
  lmer(log_rt~group*targ_index*domain*modality + (1 + modality*domain | id),
       data = rt_trial_scaled,
       contrasts = list(group = c(-0.5,0.5),
                        modality = c(-0.5, 0.5),
                        domain = c(-0.5, 0.5)),
...
lmer_slopeLING <-
  lmer(scaled_rt~group*targ_index*modality + (0+modality|id) + (1|id),
...
lmer_slopeVisual <-
  lmer(scaled_rt~group*targ_index*domain +  (0+domain|id) + (1|id),
...
asd_composite$language_age_level <-
revalue(asd_composite$language_age_level, c("signif_below_age"="below_age",
                                     "slight_below_age"="below_age",
                                     "at_age"="at_age",
                                     "above_age"="at_age"))
detach(package:plyr)

# asd_composite$language_age_level <- as.character(asd_composite$language_age_level)
# asd_composite[which(asd_composite$group == "TD"), "language_age_level"] <- "TD"

asd_composite$language_age_level <- as.factor(asd_composite$language_age_level)

asd_composite$language_age_level <- 
  factor(asd_composite$language_age_level,
         levels = c("below_age", 
                    "at_age"
                    # "TD"
                    ))
...
# asd_composite <- asd_composite[which(!is.na(asd_composite$language_age_level) &
#                                        !is.na(asd_composite$composite_ling)),]
# asd_composite <- asd_composite[which(!is.na(asd_composite$scq_web)),]
# asd_composite <- asd_composite[which(!is.na(asd_composite$rbs_r)),]

nrow(asd_composite[which(!is.na(asd_composite$a_prime) & !is.na(asd_composite$composite_ling)),])
...
t.test(asd_composite[which(asd_composite$language_age_level == "below_age"),"composite_ling"],
       asd_composite[which(asd_composite$language_age_level == "at_age"),"composite_ling"])

# library(ez)
# 
# ezANOVA(data=asd_composite, wid=part_id, dv=composite_ling, between = language_age_level,
#         type=3, detailed=TRUE)


# ezANOVA(data=asd_composite, wid=part_id, dv=composite, between = language_age_level,
#         between_covariates = c("age_at_web_year"),
#         type=3, detailed=TRUE)
```


# Multiple Regression for GJT and Composite Score
```{r}
# Regenerate a long data frame with GJT and scaled accuracy scores
gjt_mulreg <- asd_composite[,c("part_id", "group", "age_at_web_month", "age_at_web_year", "a_prime",
                               "scq_web", "rbs_r", "composite_ling", "composite_nonling")]
gjt_mulreg <- 
  melt(gjt_mulreg, id.vars= c("part_id", "group", "age_at_web_month", "age_at_web_year", "a_prime",
                               "scq_web", "rbs_r"))
...
asd_corr_posthoc <-
  rcorr.adjust(as.matrix(asd_composite[which(asd_composite$group == "ASD"),
                                       c("composite_ling",
                                         "composite_nonling",
                                         "a_prime",
                                     "rbs_r",
...
rtASD <- asd_composite[which(asd_composite$group == "ASD"),
                    c("part_id", "group", 
                   # "lsl_acc","ssl_acc",
                    # "vsl_acc","tsl_acc",
                   "lsl_slope", "ssl_slope", 
...
ggsave("asd_split_by_lang.png", width = 10,
       dpi = 150)
```
...
cast(acc_stat, task~Group, value = "ci_lower")
cast(acc_stat, task~Group, value = "ci_upper")
...
```{r}
plot_medianSplit <- function(task, titleTask) {
ssl_slope <- allData_slope[which(allData_slope$task == task & 
                                               allData_slope$group == "TD"),]
...
ssl_slopeASD <- allData_slope[which(allData_slope$task == task & 
                                               allData_slope$group == "ASD"),]
...
ssl_slope_ASDtd %>%
  ...
  geom_bar(
           ...
           width = 0.9,
           stat = "summary", fun.y = "mean") +
   labs(title = paste(titleTask, "SL RT Slope Median Split by Age in TD and ASD"),
        x = "Age Group",  # Change x-axis label
         y = paste(titleTask, "SL RT Slope (arbitrary unit/ trial)")) +
  scale_fill_manual(values = c("#F8766d", "#00BFC4", "#00BFC4")) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(
        plot.title = element_text(size=16, face="bold"),
...
plot_medianSplit("lsl_slope", "Letter")

ggsave("lsl_slope_td_asd.png",
       width = 18, height = 15, units = "cm")

plot_medianSplit("ssl_slope", "Syllable")
...
slope_within_stat %>%
  ...
  geom_text(aes(x= 0.95, y = -0.033, label = paste0("*")), size=7, color="#F8766d")
   # geom_text(aes(x= 4.05, y = -0.025, label = paste0("*")), size=9, color="black")
...
ggplot(aes(x = a_prime, y = composite_ling, color = language_age_level)) +
  ...
  # geom_smooth(data = subset(asd_composite, language_age_level == "below_age"),
  #             aes(x = scq_web, y = composite),
  #             method=lm, se=FALSE, show.legend = F, inherit.aes = F) +
  # geom_smooth(data = subset(asd_composite, language_age_level == "at_age"),
  #             aes(x = scq_web, y = composite),
  #             method=lm, se=FALSE, show.legend = F, inherit.aes = F)
...
```{r}
allData %>%
ggplot(aes(x = age_at_web_year, y = lsl_acc, color = Group)) +
  geom_point(size = 4, shape = 1, position = pd, stroke = 1.7) +
  scale_fill_brewer(palette = "Set2") +
  ...
  geom_smooth(data = subset(allData, Group == "TD"),
              aes(x = age_at_web_year, y = lsl_acc),
              method=lm, se=FALSE, show.legend = F, inherit.aes = F, color = "#00BFC4") +
  geom_smooth(data = subset(allData, Group == "ASD"),
              aes(x = age_at_web_year, y = lsl_acc),
              method=lm, se=FALSE, show.legend = F, inherit.aes = F, color = "#F8766d") +
