...
## JoVE Version 1 Submission Script

``` {r}

output_path <- ("/Users/jojohu/Documents/Qlab/JoVE/")

drive_checklist <-
  read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/drive_checklist.csv")

drive_checklist <- drive_checklist[-which(drive_checklist$Participant.ID == "blast_c_267"| 
                                          drive_checklist$Participant.ID == "blast_c_268" |
                                          drive_checklist$Participant.ID == "blast_c_269"),]
...
bucld_demo_all <- bucld_demo_all[-which(bucld_demo_all$part_id == "blast_c_267"| 
                                        bucld_demo_all$part_id == "blast_c_268" |
                                        bucld_demo_all$part_id == "blast_c_269"),]
#Read in GJT data 
...
blast_spoli_data_wide <- blast_spoli_data_wide[-which(blast_spoli_data_wide$part_id == "blast_c_267"|
                                                      blast_spoli_data_wide$part_id == "blast_c_268"|
                                                      blast_spoli_data_wide$part_id == "blast_c_269"),]
...
gjt_original_file <- read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/gjt_score.csv")
...
ssl_rt_hit_count <- read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/ssl_rt_hit_trial_count.csv")
tsl_rt_hit_count <- read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/tsl_rt_hit_trial_count.csv")
vsl_rt_hit_count <- read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/vsl_rt_hit_trial_count.csv")
lsl_rt_hit_count <- read.csv("/Users/jojohu/Documents/Qlab/bucld_2019_followup/lsl_rt_hit_trial_count.csv")
...
print(bucld_all_completed[which(bucld_all_completed$group == "TD"), 
                       c("part_id", "group")])
```

These are removed outliers: 

SSL RT and Slope < 6 hits: `r ssl_rt_outlier` + 1 ASD spoli_c_125 outlier of SSL slope
...
write.csv(age_descrip_stat, paste0(output_path, "age_descrip_stat.csv"))
```
...
print(taskcomp_count)
 ...
 write.csv(taskcomp_count, paste0(output_path, "task_comp_count.csv"))
```
...
write.csv(m1_acc_sum$coefficients, paste0(output_path, "lmer_acc.csv"))
...
write.csv(m1_slope_sum$coefficients, paste0(output_path,"lmer_slope.csv"))
write.csv(bucld_all_completed_slope, paste0(output_path, "bucld_all_completed_slope_use_this_poster.csv"))
```
...
write.csv(acc_trial, paste0(output_path, "acc_trial.csv"))
...
summary(logacc)
```
**GLMER missing Spoli data, marginally significant group main effect (?isSingular warning: optimizer didn't help.)**
...
lmerrt_trial <-
  lmer(reaction_time~group*targ_index + (1+targ_index|id), data = rt_trial)

summary(lmerrt_trial)
```
**Wrong LMER result for RT by trial: Model failed to converge with max|grad| = 0.00312988 (tol = 0.002, component 1)**
...
write.csv(acc_stat_across_sd, paste0(output_path, "acc_stat.csv"))
```
...
write.csv(blast_spoli_data_wide, paste0(output_path, "blast_spoli_data_all_measures.csv"))
write.csv(bucld_all_completed, paste0(output_path, "blast_spoli_data_all_measures_no_outlier.csv"))
```
...
write.csv(slope_stat, paste0(output_path, "slope_stat.csv"))
```
...
ggsave("behavioral_slope_ling_bar.png",
       bg="transparent",
...
geom_hline(yintercept=0, color = "red", size = 0.5, stat="density", alpha=0.6) +
geom_text(aes(x= 2, y = 0.1, label = paste0("†")), size=7, color="red") +
geom_text(aes(x= 4, y = 0.1, label = paste0("*")), size=6, color="red")
...
write.csv(ling_mulreg$coefficients,  paste0(output_path, "ling_mulreg.csv"))
...
ggsave("lsl_vsl_rtSlopeCorr.png",
       width = 10, height = 15, units = "cm")
...
accCol <- which(str_detect(colnames(bucld_all_completed_td), "\\S+(?=l_accuracies)"))
slpCol <- which(str_detect(colnames(bucld_all_completed_td), "\\S+(?=_slope)"))

accTD <- bucld_all_completed_td[,c(1:3, accCol)]
accTDlg <- melt(accTD, id.vars=c(1:3))
colnames(accTDlg)[colnames(accTDlg) == "value"] <- "accuracy"
colnames(accTDlg)[colnames(accTDlg) == "variable"] <- "task"

slpTD <- bucld_all_completed_td[,c(1:3, slpCol)]
slpTDlg <- melt(slpTD, id.vars=c(1:3))
colnames(slpTDlg)[colnames(slpTDlg) == "value"] <- "slope"
colnames(slpTDlg)[colnames(slpTDlg) == "variable"] <- "taskslp"

tdAccSlp <- cbind(accTDlg, slpTDlg[,-c(2:3)])

tdAccSlp$accuracy <- as.numeric(as.character(tdAccSlp$accuracy))
tdAccSlp$slope <- as.numeric(as.character(tdAccSlp$slope))
rcorr(as.matrix(tdAccSlp[,c("accuracy", "slope")]), type = "pearson")

tdAccSlp_lreg <- lm(accuracy~slope*task, data = tdAccSlp)
summary(tdAccSlp_lreg)
...
write.csv(beh_corr_result_pearson_sig, paste0(output_path, "beh_corr_result_pearson_gender_matched_sample.csv"))
write.csv(beh_corr_result_pearson_sig_td, paste0(output_path, "beh_corr_result_pearson_gender_matched_sample_td.csv"))
write.csv(beh_corr_result_pearson_sig_asd, paste0(output_path, "beh_corr_result_pearson_gender_matched_sample_asd.csv"))
...
write.csv(GJTrsrDec, paste0(output_path, "GJTrsrDec.csv"))
```
...
write.csv(data.frame(sl_corr$R[1]), paste0(output_path, "tdSL_peaR.csv"))
write.csv(tdSL_corr, paste0(output_path, "tdSL_corr_corrected.csv"))
```
...
write.csv(data.frame(sl_corrASD$R[1]), paste0(output_path, "asdSL_peaR.csv"))
write.csv(asdSL_corr, paste0(output_path, "asdSL_corr_corrected.csv"))
```
...
ggplot(data = heat_map_corr_data_task_jove, aes(x = column, y = row, fill = cor)) +
  ...
  scale_size(range = c(3.5, 6.5), guide = F) +
  theme(legend.text = element_text(size=15), legend.title = element_text(size=14))
...
testd <- bucld_all_completed[which(
  bucld_all_completed$accuracy_children_tsl_accuracies != "" &
    ...
    bucld_all_completed$rt_children_lsl_indiv_rts != "" &
     bucld_all_completed$rt_children_tsl_indiv_rts != "" &
     bucld_all_completed$rt_children_ssl_indiv_rts != "" &
   bucld_all_completed$rt_children_vsl_indiv_rts != ""
), ]
...
summary(pcacomp)
```
**Question: center = TRUE, scale. = TRUE, How does it differ from our own scaling? There is a slight difference in results, 0.4762 vs.  0.4857 (our own scaling). Is it bc the input file is different? crossed_random_effect vs. bucld_all_complete.**

**60% first component comes from dataset that removes missing rows for accuracy and for RT slope.**
...
pca_test <- prcomp(~ ., bucld_all_completed[,c("accuracy_children_lsl_accuracies", 
                                              ...
                                              "rt_children_ssl_indiv_rts",
                                              "rt_children_vsl_indiv_rts",
                                              "rt_children_tsl_indiv_rts")], center = TRUE,scale. = TRUE, na.action=na.omit)
...
library(ggbiplot)
ggbiplot(pca_test, labels=testd$part_id, ellipse=TRUE,  groups=testd$group)
```
